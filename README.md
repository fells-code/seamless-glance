...
- the initial finding set includes alarms in `ALARM`, stopped EC2 instances, RDS instances that are not available, Lambda functions with high memory or stale deploy dates, secrets without rotation, target groups with zero healthy targets, target groups with unhealthy targets, SQS queues without DLQs, security groups open to the world, and security groups exposing sensitive ports publicly
